...
module thl_vetoken::thl_vetoken_package {
    ...
    //                                                                        //
    const DEPLOYER_ADDRESS: address = @thl_vetoken_deployer;
    const RESOURCE_ACCOUNT_SEED: vector<u8> = b"thl_vetoken";
    const RESOURCE_ACCOUNT_ADDRESS: address = @thl_vetoken;
    //                                                                        //
    //                                                                        //
    //                                                                        //
    // *************************************************************************
    ...
    use resource_signer_package::package::SignerKey;
    ...
    public entry fun fetch_signer_key(manager: &signer) {
        assert!(thala_manager::manager::is_authorized(manager), ERR_PACKAGE_UNAUTHORIZED);
        let signer_key = resource_signer_package::package::create_signer_key(manager);
        let resource_signer = resource_signer_package::package::resource_account_signer_with_key(&signer_key);
        move_to(&resource_signer, ResourceSignerCapability{
    ...
    public(friend) fun resource_account_signer(): signer acquires ResourceSignerCapability {
        ...
        let ResourceSignerCapability { signer_key } = borrow_global<ResourceSignerCapability>(resource_account_address);
        resource_signer_package::package::resource_account_signer_with_key(signer_key)
    }
